...
const Post = tw.a`flex flex-col h-full bg-gray-200 rounded bg-gray-800 text-gray-100`;
const PostImage = styled.div`
  ${props => css`background-image: url("${props.imageSrc}");`}
  ${tw`h-64 sm:h-80 bg-center bg-cover rounded-t`}
`;
const PostText = tw.div`flex-1 px-6 py-8` 
const PostTitle = tw.h6`font-bold group-hocus:text-primary-500 transition duration-300 text-gray-100 `;
const PostDescription = tw.p``;
const AuthorInfo = tw.div`flex`;
const AuthorImage = tw.img`w-12 h-12 rounded-full mr-3`;
const AuthorTextInfo = tw.div`text-xs text-gray-400`;
const AuthorName = tw.div`font-semibold mt-2 text-gray-500`;
...
const PostContainer = styled.div`
  ...
  ${props => props.featured && css`
    ...
    ${PostDescription} {
      ${tw`mt-4 text-sm font-semibold text-gray-600 leading-relaxed text-gray-400`}
    }
    ${AuthorInfo} {
      ${tw`mt-8 flex items-center text-gray-600`}
    }
    ${AuthorName} {
      ${tw`mt-0 font-bold text-gray-700 text-sm text-gray-500`}
    }
  `}
`;
...
export default ({
  ...
  description = "Read the Most Recent Blog Posts.",
  posts = [
    {
      postImageSrc:
